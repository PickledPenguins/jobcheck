...
lives on ``main`` under ``recovery/bytecode/`` rather than on this branch: fetch
it with ``git restore --source=origin/main -- recovery``. A ``.pyc`` still
carries every code object, so class and function names, their parameters, their
annotations and their docstrings survive even though the statements do not. This
script reads those out with ``marshal`` and prints them as Markdown, which is how
``recovery/recovered-api.md`` was produced.
...
#: Where the check-era bytecode lands when it is restored from ``main``. It is
#: not on this branch, so the directory is an argument rather than a default
#: that would usually name nothing.
RESTORED_DIR = Path(__file__).resolve().parent.parent / "recovery" / "bytecode" / "jobcheck"
...
def main(argv: list[str]) -> int:
    directory = Path(argv[1]) if len(argv) > 1 else RESTORED_DIR
    if not directory.is_dir():
        print(f"No such directory: {directory}", file=sys.stderr)
        if directory == RESTORED_DIR:
            print(
                "The check-era bytecode is on main, not on this branch: "
                "git restore --source=origin/main -- recovery",
                file=sys.stderr,
            )
        return 2
