...
the part of the surface that rots quietly: a reworded message breaks no test that
...
from pandas_row_validation.results import Status, TestResult, normalise_result
...
def test_an_empty_code_says_what_a_code_must_be(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="", message="m")(lambda row: True)
    assert message_of(raised) == "Test code must be a non-empty string, got ''."


def test_a_missing_message_says_what_the_message_is_for(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="CODE", message="")(lambda row: True)
    assert message_of(raised) == "Test 'CODE': message must be the text a person sees on failure."


def test_a_duplicate_code_names_what_registered_it(fresh_registry: None) -> None:
    make_test("TAKEN")
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="TAKEN", message="m")(lambda row: True)
    assert "Duplicate test code 'TAKEN' (registering " in message_of(raised)
...
def test_a_string_depends_on_explains_why_it_is_wrong(fresh_registry: None) -> None:
    """Regression: register_test used to do list(depends_on or []) before the guard
    ...
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="CODE", message="m", depends_on="OTHER")(  # type: ignore[arg-type]
            lambda row: True)
    assert message_of(raised) == (
        "Test 'CODE': depends_on must be a list of test codes, got 'OTHER'. "
...
def test_a_string_depends_on_in_a_group_says_the_same(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.test_group(depends_on="OTHER")  # type: ignore[arg-type]
    assert message_of(raised) == (
        "test_group depends_on must be a list of test codes, got 'OTHER'. "
        "A bare string is a list of its characters, which is never what you meant."
    )
...
def test_a_non_boolean_default_enabled_names_the_value(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="CODE", message="m", default_enabled="yes")(  # type: ignore[arg-type]
            lambda row: True)
    assert message_of(raised) == (
        "Test 'CODE': default_enabled must be True or False, got 'yes'.")


def test_a_non_text_description_names_the_value(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="CODE", message="m", description=7)(  # type: ignore[arg-type]
            lambda row: True)
    assert message_of(raised) == "Test 'CODE': description must be text, got 7."


def test_a_non_text_suite_says_what_to_do_instead(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.register_test(code="CODE", message="m", suite=7)(  # type: ignore[arg-type]
            lambda row: True)
    assert message_of(raised) == (
        "Test 'CODE': suite must be the name of a suite, got 7. Leave it out "
...
def test_a_three_argument_test_is_rejected_with_its_signature(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        ...
        def check(row, ctx, extra):  # type: ignore[no-untyped-def]
            return True
    assert message_of(raised) == (
        "Test 'CODE': check(row, ctx, extra) must take (row) or (row, ctx), "
...
def test_a_required_keyword_argument_says_how_to_fix_it(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        ...
        def check(row, *, limit):  # type: ignore[no-untyped-def]
            return True
    assert message_of(raised) == (
        "Test 'CODE': check(row, *, limit) needs keyword argument(s) "
...
def test_a_dangling_prerequisite_lists_the_loaded_suites(fresh_registry: None) -> None:
    make_test("DEPENDENT", depends_on=["ABSENT"])
    with pytest.raises(ValueError) as raised:
        reg.validate_registry()
    assert message_of(raised) == (
        "Test 'DEPENDENT' depends on 'ABSENT', which is not registered. "
...
def test_a_missing_test_file_says_nothing_is_discovered(fresh_registry: None) -> None:
    with pytest.raises(ValueError) as raised:
        reg.load_test_files(["/no/such/file.py"])
    assert message_of(raised) == (
        "No test file at '/no/such/file.py'. load_test_files() names files "
...
def test_duplicate_column_labels_say_what_a_test_would_receive(fresh_registry: None) -> None:
    ...
    with pytest.raises(ValueError) as raised:
        explain_row(row)
    assert message_of(raised) == (
        "Row has duplicate column labels ['age']: a test reading one of them would "
...
def test_a_test_returning_nonsense_says_what_it_may_return(fresh_registry: None) -> None:
    ...
    assert message_of(raised).startswith("Test 'CODE' returned ")
    assert message_of(raised).endswith(
        "A test must return PASS, a TestResult, a bool, or a Status value.")
...
def test_from_records_of_the_wrong_length_says_what_it_needs(fresh_registry: None) -> None:
    from pandas_row_validation import ValidationRun
