...
CRASHES = """
from jobcheck import register_check
from jobcheck import engine

@register_check("BOOM", "b is above the limit")
def boom(row):
    return 1 / 0
"""
...
def test_a_passing_row_has_no_failures(fresh_registry: None, tmp_path: Path) -> None:
    load_checks([write_file(tmp_path, SIMPLE)])
    assert failures(validate(frame({"a": "x", "b": "1"}))[0]) == []
...
def test_a_failing_row_reports_the_code_message_and_comments(
    ...
    load_checks([write_file(tmp_path, SIMPLE)])
    (failure,) = failures(validate(frame({"a": "x", "b": "zz"}))[0])
...
def test_failures_line_up_with_the_rows_they_came_from(
    ...
    load_checks([write_file(tmp_path, SIMPLE)])
    rows = validate(frame({"a": "x", "b": "1"}, {"a": "y", "b": "zz"}, {"a": "z", "b": "3"}))
    assert [bool(failures(row)) for row in rows] == [False, True, False]
...
def test_a_blank_column_produces_one_failure_not_three(
    ...
    row = validate(frame({"a": "x", "b": ""}))[0]
    assert [failure.code for failure in failures(row)] == ["B_PRESENT"]
...
def test_a_cross_row_test_sees_the_whole_file(fresh_registry: None, tmp_path: Path) -> None:
    ...
    assert [failures(row)[0].code for row in rows] == ["A_UNIQUE", "A_UNIQUE"]
    assert failures(rows[0])[0].comments["n"] == 2
...
def test_the_population_can_come_from_rows_that_are_not_being_validated(
    ...
    rows = validate(frame({"a": "taken", "b": "1"}),
