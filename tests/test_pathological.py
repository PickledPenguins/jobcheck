...
def test_a_pattern_matching_a_unicode_value(fresh_registry: None, tmp_path: Path) -> None:
    ...
    assert reg.resolve_enabled_state(pd.Series({"city": "MÃ¼nchen"}), rules)["A_CODE"] is False
...
def test_empty_row_reports_the_presence_tests_and_nothing_below_them(
    ...
    assert [r.code for r in reg.validate_row(pd.Series(dtype=object))] == [
...
def test_duplicate_labels_are_rejected_before_any_check_runs(fresh_registry: None) -> None:
    calls: list[str] = []
    make_test("A_CODE", calls=calls)
    with pytest.raises(ValueError):
        reg.validate_row(pd.Series([1, 2], index=["age", "age"]))
    assert calls == []
...
def test_a_very_long_string_value_is_matched_not_truncated(fresh_registry: None) -> None:
    ...
    assert reg.resolve_enabled_state(row, [rule])["A_CODE"] is False
...
def test_a_test_that_raises_is_recorded_as_an_error_not_a_pass(fresh_registry: None) -> None:
    ...
    @reg.register_test(code="EXPLODES", message="m")
    def check(row: "pd.Series[Any]") -> bool:
        raise RuntimeError("test is broken")

    outcome = reg.validate_row(pd.Series({"age": 1}))[0]
    assert outcome.outcome == "errored"
    assert outcome.status == Status.ERROR
    assert outcome.detail == "RuntimeError: test is broken"
...
def test_a_raising_test_can_be_made_fatal(fresh_registry: None) -> None:
    ...
    with pytest.raises(RuntimeError, match="test is broken"):
        reg.validate_row(pd.Series({"age": 1}), on_error="raise")
...
def test_a_test_reading_a_column_that_is_absent_errors_naming_it(fresh_registry: None) -> None:
    ...
    @reg.register_test(code="TYPO", message="m")
    def check(row: "pd.Series[Any]") -> bool:
        return row["agee"] > 0
...
def test_a_test_returning_nothing_raises_even_when_errors_are_recorded(
    ...
    @reg.register_test(code="FORGOT", message="m")
    def check(row: "pd.Series[Any]") -> Any:
        pass

    with pytest.raises(TypeError, match=r"Test 'FORGOT' returned None"):
        reg.validate_row(pd.Series({"age": 1}))


def test_a_deep_dependency_chain_evaluates_in_order(fresh_registry: None) -> None:
    calls: list[str] = []
    make_test("LINK_000", passes=True, calls=calls)
    for i in range(1, 200):
        make_test(f"LINK_{i:03d}", passes=True, depends_on=[f"LINK_{i - 1:03d}"], calls=calls)
...
def test_a_deep_chain_skips_everything_below_a_failure(fresh_registry: None) -> None:
    calls: list[str] = []
    make_test("LINK_000", passes=False, calls=calls)
    for i in range(1, 200):
        make_test(f"LINK_{i:03d}", passes=True, depends_on=[f"LINK_{i - 1:03d}"], calls=calls)
    assert [r.code for r in reg.validate_row(pd.Series({"age": 1}))] == ["LINK_000"]
...
def test_a_key_column_value_containing_the_separator_is_left_as_it_is(
    fresh_registry: None,
) -> None:
    ...
    report = build_report(collect_outcomes(frame), df=frame, key_column=["batch", "id"])
    assert list(report["row"]) == ["A|B|1"]


def test_a_frame_with_duplicate_column_labels_fails_on_the_first_row(
    fresh_registry: None,
) -> None:
    from pandas_row_validation import collect_outcomes
...
def test_an_empty_frame_produces_an_empty_report(fresh_registry: None) -> None:
    from pandas_row_validation import build_report, collect_outcomes, summarise_outcomes
