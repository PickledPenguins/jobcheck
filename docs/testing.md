...
| `tests/test_docs_unit.py` | The rest of `docs/`, both directions: every call shown binds against the real signature, every exported name appears in `interfaces.md` and nothing documented there is gone, every check code a document shows is one that exists, every exit code the entry point can return has a row in `docs/cli.md` and no row describes one it cannot, the README stays an index and links every document, no internal link or anchor is dead, and the rule keys, statuses and outcome names are documented where they belong. |
