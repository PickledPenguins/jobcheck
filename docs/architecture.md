...
entry point
  ...
  |       disabled / blocked  -> TestOutcome, fn never called
  |       fn(row, ctx)        -> TestResult -> TestOutcome(passed|failed)
  |       fn raises           -> TestOutcome(errored, Status.ERROR)
  |
...
```
src/pandas_row_validation/   the package: the only thing that ships
examples/                    two demo entry points, the rule files they load,
                             and example_suites/ -- the tests they run
...
**Suite is inferred from the module path, never passed.** A test file cannot then
disagree with where it lives. `pandas_row_validation.hard_tests.test_age` gives `hard_tests`; a
...
subset without interfering. Rejected: importing every `test_*.py` on package import, which
...
**`clear_registry` evicts the modules that registered tests.** Python caches a module
...
pulled in by any route — a test file importing it directly, for instance — is still
tracked.

**Tests read the row; there is no declared column.** These rules are row-scoped
...
raises `KeyError` from the test and lands as an `ERROR` outcome naming the
column. `check_rule_columns` still covers the rule side, where nothing else
would catch it.

**A test returns a status and comments, not a bool.** "Age is out of range" is
...
set can be grouped and counted across every test in a summary, which per-test
...
reporting needs to know why a test did *not* run, which means recording disabled,
...
someone is trying to understand. The cost is an object per test per row, which is
why the report path is documented as the expensive one.

**A raising test is recorded, not fatal.** One broken test should not kill a long
...
it. A test returning a nonsense value still raises unconditionally: that is an
authoring bug, and recording it would hide it.

**Layer is computed, not declared.** An author states what a test depends on,
...
X" would be worthless if any test could quietly opt out; a test that must run
regardless belongs outside the group. The cost is that a presence test cannot sit
...
**The library ships no tests of its own.** The example suites live under
...
defaulted, which is the honest signature -- the tests being loaded are always
someone else's.
...
and the `test_*.py` files are separate because their location is the suite mechanism.

## Extension points

- **A check**: a function in a `test_*.py` file under a suite subpackage. Nothing else.
...
- Regex matching stringifies values, so numeric or datetime criteria match the text of the
  value.
- A test reading a column the frame lacks raises `KeyError`, which lands as an `ERROR`
  outcome per row rather than being caught once before the run; nothing declares which
  columns a test reads, so nothing can check them up front.
- Collecting outcomes keeps an object per test per row, so the report path costs memory
  proportional to tests x rows; `validate_row` alone does not.
